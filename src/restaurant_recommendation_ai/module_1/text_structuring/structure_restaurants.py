...
for i, restaurant_paragraph in enumerate(restaurant_list):
    ...
    candidate_json_output = safe_llm_call(
        system_msg=base_system_msg, prompt_txt=base_user_prompt
    )
    ...
    while True:
        try:
            ...
            repair_system_msg, repair_prompt = JSON_auto_repair_prompts(
                candidate_json_output=candidate_json_output, error_message=error_message
            )

            candidate_json_output = safe_llm_call(
                system_msg=repair_system_msg, prompt_txt=repair_prompt
            )
...
output_path = Path("data/processed/restaurants.json")

output_path.parent.mkdir(parents=True, exist_ok=True)

with open(output_path, "w", encoding="utf-8") as file:
    json.dump(structured_restaurant_lists, file, ensure_ascii=False, indent=4)

print(f"Saved {len(structured_restaurant_lists)} restaurants to {output_path}")
